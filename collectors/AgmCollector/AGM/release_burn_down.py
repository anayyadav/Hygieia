	...
	for j in range(len(app)):
		for i in range(len(release_data)):
			...
			if(j == 0):
				...
				rb_["today"] = datetime.today().strftime('%Y-%m-%d') 
			...
			r_["today"]=datetime.today().strftime('%Y-%m-%d')
			sp = get_data(backlog_data,release_data[i]['id'])
			r_['remainingSP']=sp
			status=getstatus(backlog_data,release_data[i]['id'],url,app[j]["id"])
			r_['status']=status
...
def getreleaseforecast(release_data,backlog_data,workspace_name,workspaceid):
	conn = MongoClient(config.Agilemanager_collector['db_host'],config.Agilemanager_collector['db_port'])
	#int "Could not connect to MongoDB"
	...
	for j in range(len(release_data)):
		...
		s_['workspaceid'] = str(workspace_name)+"-"+str(workspaceid)
		for i in range(len(backlog_data)):
			if not(backlog_data[i]["story_points"] == None) and not(backlog_data[i]["release_id"] == None):
				total = total + backlog_data[i]["story_points"]
				if(str(backlog_data[i]["release_id"]["id"]) == str(release_data[j]['id']) and (backlog_data[i]["status"] == "Done") ):
					doneSP = doneSP + backlog_data[i]["story_points"]
		s_['releasescope'] = total
		s_['donesp'] = doneSP
		s_['today'] = datetime.today().strftime('%Y-%m-%d')
		s_['startdate'] =release_data[j]['start_date']
		s_['enddate']= release_data[j]['end_date']
		#s.append(s_)
		myquery = { "releaseid": str(release_data[j]['name'])+"-"+str(release_data[j]['id']), "today" : datetime.today().strftime('%Y-%m-%d') }
		collection.delete_many(myquery)
		collection.insert(s_)

(function () {
    ...
    angular
        ...
        .controller('RepomViewController', RepomViewController);

    RepomViewController.$inject = ['$q', '$scope', 'mendixData', '$uibModal'];
    function RepomViewController($q, $scope, mendixData, $uibModal) {
        var ctrl = this;
        var commitData = [];
		var resultData=[];
        ...
        function removeDumplicateValue(myArray) {
            ...
            angular.forEach(myArray, function (value, key) {
                ...
                angular.forEach(newArray, function (val2, key) {
                    if (angular.equals(value.scmRevisionNumber, val2.scmRevisionNumber)) { exists = true };
                });
                if (exists == false && value.scmRevisionNumber != "") { newArray.push(value); }
            });
        ...
        ctrl.load = function () {
            ...
            mendixData.details(params).then(function (data) {
                commitData = data.data[0].teamserverApiRevisionDetail.filter(function (item) {
        ...
        function processCommitResponse(data, numberOfDays) {
			...
			{   
				...
				_(data1).forEach(function (commit) {
					if (commit.scmCommitTimestamp >= date.getTime()) {            
						countCommit=countCommit +1 ;
						groupedCommit.push(commit);
					}
				});
				
				for(var j =0 ; j < groupedCommit.length; j++ )
				{
					for (var k =0 ; k < data1.length; k++ )
					{
						if(groupedCommit[j].scmRevisionNumber ==  data1[k].scmRevisionNumber)
						{
							//indexes.push(k);
							data1.splice(k, 1);
						}
				...
				//console.log(indexes)
				output.push({'commitcount':countCommit,'data':groupedCommit});
				
			}
			 
			
			for (var x =(output.length)-3 ; x >=0; x--) {
				
				commits.push(output[x]['commitcount']);
				groupedCommitData.push(output[x]['data']);

...
(function () {
    ...
    angular
        ...
        .controller('BuildmWidgetViewController', BuildmWidgetViewController);

    BuildmWidgetViewController.$inject = ['$scope', 'mendixData', 'DisplayState', '$q', '$uibModal'];
    function BuildmWidgetViewController($scope, mendixData, DisplayState, $q, $uibModal) {
        var ctrl = this;
        var builds = [];
        ...
        ctrl.load = function() {
            ...
            mendixData.details(params).then(function(data) {

                builds = data.data[0].buildApiDetails.filter(function (item) {
        ...
        ctrl.detail = function(build) {
            $uibModal.open({
                templateUrl: 'components/widgets/mendixbuild/detail.html',
                controller: 'BuildmWidgetDetailController',
        ...
        function processResponse(data) {
			 ...
			 function averageBuildDuration(data, buildThreshold, cb) {
                ...
                function filter(data) {
                    return _.filter(data, function (item) {
                        return item.buildStatus == 'Succeed' && Math.floor(moment(item.endTime).endOf('day').diff(moment(new Date()).endOf('day'), 'days')) >= -15;
                    });
                }
            ...
            function buildsPerDay(data, cb) {
                ...
                function all(data) {
                    return _.filter(data, function (build) {
                        return build.startTime >= fifteenDays.getTime() && (build.buildStatus !== 'InProgress') && (build.buildStatus !== 'Queued') && (build.buildStatus !== 'Building');
                    });
                }
                
                function failed(data) {
                    return _.filter(data, function (build) {
                        return build.startTime >= fifteenDays.getTime() && (build.buildStatus !== 'Succeeded') && (build.buildStatus !== 'InProgress') && (build.buildStatus !== 'Queued') && (build.buildStatus !== 'Building');
                    });
                }
                ...
                function countBuilds(data) {
                    ...
                    var grouped = _.groupBy(data, function (build) {
                        return toMidnight(new Date(build.startTime)).getTime();
                    });
            ...
            function latestBuilds(data, cb) {
                ...
                data = _.sortBy(data, 'startTime').reverse().slice(0, 5);
            ...
            function setDisplayToErrorState(data, failureThreshold, cb) {
                // order by end time and limit to last 5
                data = _.sortBy(data, 'startTime').reverse().slice(0, failureThreshold);
                
                data = _.filter(data, function (item) {
                    return (item.buildStatus.toLowerCase() != 'succeeded') &&  (item.buildStatus.toLowerCase() != 'queued') &&  (item.buildStatus.toLowerCase() != 'building') &&  (item.buildStatus.toLowerCase() != 'uploading') ;
                });
            ...
            function totalBuilds(data, cb) {
                ...
                function countToday() {
                    return _.filter(data, function (build) {
                        return build.startTime >= today.getTime();
                    }).length;
                }

                function countSevenDays() {
                    return _.filter(data, function (build) {
                        return build.startTime >= sevenDays.getTime();
                    }).length;
                }

                function countFourteenDays() {
                    return _.filter(data, function (build) {
                        return build.startTime >= fourteenDays.getTime();
                    }).length;
                }
